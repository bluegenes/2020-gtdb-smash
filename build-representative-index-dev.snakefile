    ...
    for alpha, alphainfo in info["alphabet"].items():
        if "lca" in index_types:
            ...
            if gather_accessions:
                ...
                gather_targets+=expand(os.path.join(gather_dir, "{sample}", "{alphabet}", "k{k}", "{acc}_x_{sample}.{alphabet}_scaled{scaled}_k{k}.gather_summary.csv"), acc=gather_accessions, sample=sample, alphabet=alpha, scaled=alphainfo["scaled"], k=alphainfo["ksizes"])
...
rule gather_to_tax:
    input:
        gather_csv = rules.lca_gather.output.csv,
...
# rule assess_gather_to_tax
#, real taxo lineage + top match from gather_to_tax @ each rank
